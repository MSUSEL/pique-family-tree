...
export function TreeDisplay(props) {
  ...
  // start of showCanvas, end at 1166
  const showCanvas = () => {
          ...
          svg
            ...
            .text(
              treeNodes[aspect].json_data.weights[
                p_factors[factor].name
              ]//.toFixed(2) // toFixed is breaking it because it's not getting a valid argument
            );
        }
      }
    }
    ...
    if (measureWithParentsShowing !== null) {
      ...
      for (let pf in p_factors) {
        if (
          ...
          // START LOOKING HERE state.ts useProcessedData.tsx
          svg
            .append("path")
            .attr(
              "id",
